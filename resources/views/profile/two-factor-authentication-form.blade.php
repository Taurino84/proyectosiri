<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Agregue seguridad adicional a su cuenta utilizando la autenticación de doble factor.') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Finalizar la habilitación de la autenticación de dos factores.') }}
                @else
                    {{ __('Ha habilitado la autenticación de dos factores.') }}
                @endif
            @else
                {{ __('No ha habilitado la autenticación de doble factores.') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Cuando se habilita la autenticación de doble factore, se le pedirá un token seguro y aleatorio durante la autenticación. Puede recuperar este token desde la aplicación Google Authenticator de su teléfono.') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __('Para terminar de habilitar la autenticación de doble factor, escanee el siguiente código QR con la aplicación de autenticación de su teléfono o ingrese la clave de configuración y proporcione el código OTP generado.') }}
                        @else
